...
void version()
{
    ...
    cout << "kmdx " << buildinfo->project_version << "\n";
...
//  double blockFees = 15.0;
    ...
    if (!configJSON.empty())
    {
        ...
        catch (...)
        {
            ...
            cerr << "sample: \n" << genesisInfo(eth::Network::KomodoNetworkTest) << "\n";
            return 0;
        }
    }
    ...
    dev::WebThreeDirect web3(WebThreeDirect::composeClientVersion("kmdx"), getDataDir(),
            ...
            if (!keyManager.load(masterPassword) && masterSet)
            {
                while (true)
                {
                    ...
                    if (keyManager.load(masterPassword))
                        break;
                    cout << "The password you entered is incorrect. If you have forgotten your password, and you wish to start afresh, manually remove the file: " << (getDataDir("kmdx") / fs::path("keys.info")).string() << "\n";
                }
            }
        }
        else
